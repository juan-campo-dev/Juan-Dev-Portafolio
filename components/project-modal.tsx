  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 md:p-4">
      ...
      <div className="relative z-10 w-full max-w-7xl max-h-[95vh] overflow-y-auto rounded-2xl">
        ...
        <div className="relative bg-black/85 backdrop-blur-xl rounded-2xl border border-[#00f0ff]/20 shadow-2xl min-h-fit overflow-hidden">
          {/* Header */}
          <div className="flex items-center justify-between h-12 md:h-14 px-3 md:px-6 bg-gradient-to-r from-[#00f0ff]/10 via-transparent to-[#00f0ff]/10 border-b border-[#00f0ff]/20 rounded-t-2xl">
            <div className="flex space-x-1 md:space-x-2">
              {[0, 1, 2].map(i => (
                <div key={i} className={`w-2 h-2 md:w-3 md:h-3 rounded-full opacity-80 shadow-lg ${["bg-red-500", "bg-yellow-500", "bg-green-500"][i]}`} />
              ))}
            </div>
            <span className="text-[#c2e9fa] font-mono text-sm md:text-xl lg:text-2xl font-bold tracking-wider truncate mx-2 max-w-[60%] md:max-w-none" title={title}>{title}</span>
            <button onClick={handleClose} aria-label="Cerrar" className="group relative p-1 md:p-2 text-white/70 hover:text-[#00f0ff] transition-all duration-300 flex-shrink-0">
              <div className="absolute inset-0 bg-[#00f0ff]/20 rounded-full scale-0 group-hover:scale-100 transition-transform duration-300" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6 p-3 md:p-6 pb-6">
            ...
            <div className="lg:col-span-2 flex flex-col items-center space-y-4 md:space-y-6 w-full">
              <Suspense fallback={<div className="text-white">Cargando...</div>}>
                <div className="relative group flex items-center justify-center w-full">
                  <div className="absolute -inset-0.5 bg-gradient-to-br from-[#00f0ff]/10 via-[#001a2b]/40 to-[#0080ff]/10 rounded-xl border border-[#00f0ff]/10 shadow-[0_0_16px_2px_#00f0ff22]" />
                  <div className="relative bg-black/80 rounded-xl overflow-hidden flex items-center justify-center border border-[#00f0ff]/10 shadow-md w-full h-48 md:h-80 lg:h-96">
                    <Swiper
              ...
              <div className="flex flex-wrap justify-center items-center gap-2 md:gap-4 w-full max-w-md">
                ...
                {isPrivate && (
                  ...
                  >
                    <Github className="h-3 w-3 md:h-4 md:w-4 mr-1 md:mr-2" /> Privado
                  </Button>
                )}
                ...
                {demoSoon && (
                  <Button
                    ...
                    className="inline-flex items-center justify-center bg-gray-600 text-white cursor-not-allowed opacity-60 rounded-full px-3 md:px-4 py-2 tracking-normal text-sm"
                  >
                    <ExternalLink className="h-3 w-3 md:h-4 md:w-4 mr-1 md:mr-2" /> ¡Próximamente!
                  </Button>
                )}
                {demoPrivate && (
                  <Button
                    ...
                    className="inline-flex items-center justify-center bg-gray-600 text-white cursor-not-allowed opacity-60 rounded-full px-3 md:px-4 py-2 tracking-normal text-sm"
                  >
                    <ExternalLink className="h-3 w-3 md:h-4 md:w-4 mr-1 md:mr-2" /> Restringido
                  </Button>
                )}
              </div>
            </div>
            ...
            <div className="lg:col-span-1 flex flex-col space-y-4 md:space-y-6">
              <div className="bg-black/40 backdrop-blur-sm border border-[#00f0ff]/20 rounded-xl p-4 md:p-6">
                <div className="text-xs md:text-sm text-[#00f0ff] font-mono mb-2 md:mb-3 tracking-wider">DESCRIPCIÓN</div>
                <p className="text-gray-300 leading-relaxed text-xs md:text-sm">{description}</p>
              </div>
              <div className="bg-black/40 backdrop-blur-sm border border-[#00f0ff]/20 rounded-xl p-4 md:p-6">
                <div className="text-xs md:text-sm text-[#00f0ff] font-mono mb-3 md:mb-4 tracking-wider">TECNOLOGIAS IMPLEMENTADAS</div>
